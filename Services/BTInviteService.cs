        ...
        public async Task<bool> AcceptInviteAsync(Guid? token, string userId, int companyId)
        {
            try
            {
                Invite? invite = await _context.Invites
                                               .FirstOrDefaultAsync(i => i.CompanyToken == token && i.CompanyId == companyId && i.IsValid == true);
        ...
        public async Task<Invite?> GetInviteAsync(Guid token, string email, int companyId)
        {
            try
            {
                Invite? invite = await _context.Invites
                    ...
                    .Include(i => i.Invitee)
                    .Include(i => i.Project)
                    .FirstOrDefaultAsync(i => i.CompanyToken == token && i.CompanyId == companyId && i.InviteeEmail == email);
